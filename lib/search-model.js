...
module.exports = class SearchModel {
  ...
  constructor(vimState, options) {
    ...
    this.onDidChangeCurrentMatch(() => {
      ...
      if (!this.currentMatch) {
        ...
        if(settings.get("lastSuccesfulMatch")) {
            this.currentMatch = this.lastCurrentMatch
        }
        return
      }
  ...
  getVisibleMatchRanges() {
    ...
    return visibleRange ? this.matches.filter(range => range.intersectsWith(visibleRange)) : []
  }
